// Edge Function: admin-questionnaire-windows-upsert
// Creates or updates a questionnaire window (admin-only)
...
serve(async (req) => {
  ...
  try {
    ...
    if (!isValidIsoDateTime(body?.starts_at) || !isValidIsoDateTime(body?.ends_at)) {
      return new Response(JSON.stringify({ error: 'Invalid window time range' }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 400,
      })
    }
    ...
    if (error) {
      console.error('Database upsert error (questionnaire_windows):', error)
    ...
    return new Response(JSON.stringify({ window: windowRow }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      status: 200,
    })
  } catch (error) {
    console.error('admin-questionnaire-windows-upsert error:', error)
    const errorMessage = error instanceof Error ? error.message : String(error)
    return new Response(JSON.stringify({ error: errorMessage || 'Failed to upsert window' }), {
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      status: 500,
    })
  }
